	...
	public CompletableFuture<Void> apply(Context context, Message message) {
		if (!inCheckoutMode(context)) { // Accept external messages
			...
			if (message.is(ORDER_CREATE_JSON_TYPE)) {
				...
				INTERNAL_PAYMENT_CANCEL)) { // Can be used internally when checkout fails, not using this one
			...
			if (message.is(INTERNAL_STOCK_CHECKOUT_CALLBACK)) {
				...
				if (internalMessage.isOk()) {
					...
					if (!isRestockSent(context)) {
						...
						context.send(
							KafkaEgressMessage.forEgress(KAFKA_EGRESS)
								.withTopic("egress-order-checkout")
								.withUtf8Key(context.storage().get(UID).orElse(-1).toString())
								.withValue(EGRESS_CHECKOUT_SUCCESS_TYPE, egressCheckoutStatus)
								.build());
				...
				if (internalOrderPayMessage.isPaid()) {
					...
					EgressCheckoutStatus egressCheckoutStatus = new EgressCheckoutStatus(context.storage().get(UID).orElse(-1), true);
